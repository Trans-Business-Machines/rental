...
export function InventoryAssignmentsList({ assignments, onUpdate, showReturnButton = true }: InventoryAssignmentsListProps) {
	...
	const handleReturn = async (assignmentId: number) => {
		...
		try {
			await returnInventoryAssignment(assignmentId, "Returned via web interface");
					...
					<TableBody>
						{assignments.map((assignment) => (
							<TableRow key={assignment.id}>
								<TableCell>
									<div>
										<div className="font-medium">{assignment.inventoryItem.itemName}</div>
										<div className="text-sm text-muted-foreground">{assignment.inventoryItem.category}</div>
									</div>
								</TableCell>
								<TableCell>
									<div className="flex items-center gap-2">
										...
										<div>
											...
											{assignment.unit?.name && (
												<div className="text-sm text-muted-foreground">{assignment.unit.name}</div>
											)}
										</div>
									</div>
								</TableCell>
