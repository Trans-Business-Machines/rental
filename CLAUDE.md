...
- Pricing: `PriceDuration` is `one_night | weekly | monthly | custom`; use the `getDuration*`/`calculate*` helpers in [lib/utils.ts](lib/utils.ts) (nights, discounted price, total) rather than recalculating inline. There is **no tax/VAT** — the price a superAdmin sets is the price displayed and charged. Note `unitPrice` is persisted already-discounted, so call `calculateTotalAmount(unitPrice, period)` without a `discountRate` argument when deriving a total from a saved booking.
- Booking/request totals are derived for display (`unitPrice × period`) rather than read from the stored `totalAmount`, because rows created before VAT was removed still have a 16%-inclusive `totalAmount`. New records store the VAT-free value.
- **Currency:** every monetary column in the DB is **KES** — that is the base unit for all storage and arithmetic. USD is a display/input layer only, converted at the fixed `NEXT_PUBLIC_USD_TO_KES_RATE` (default 130) via [lib/currency.ts](lib/currency.ts). The viewer's preference (`"USD" | "KSH"`, default USD) lives in `localStorage` behind `CurrencyProvider`/`useCurrency()`.
  - Render prices with `<Price kes={…} />` ([components/Price.tsx](components/Price.tsx)) — it's a client component, so **server components can render it too**. Don't call `formatPrice` directly in UI; it's now just the KES formatter that `formatMoney` builds on.
  - The unit-pricing form takes **USD** and calls `usdToKes()` before saving; the edit dialog prefills with `kesToUsd()`. Booking maths (`calculateTotalAmount` etc.) stays entirely in KES.
  - Server-rendered output with no viewer preference (emails, the checkout cron) uses `formatBoth()` → `"$138.46 (Ksh 18,000)"`.
...
- `NEXT_PUBLIC_USD_TO_KES_RATE`, `CRON_SECRET`, `BETTER_AUTH_SECRET`/`BETTER_AUTH_URL`, `DATABASE_URL`, `RESEND_API_KEY`/`EMAIL_FROM`, and Supabase keys are the operative env vars — see `.env` for the full set (not committed).
